...
try {
    ...
    $max_km = isset($_POST['max-kilometer']) ? (int)$_POST['max-kilometer'] : 100000;

    // New CC filter added
    $engine_displacement = isset($_POST['engine_displacement']) ? trim($_POST['engine_displacement']) : '';

    // Location filter is kept based on input or default (not passed through URL)
    $location = isset($_POST['location']) ? trim($_POST['location']) : '';  // User input for location

    // Brand filter passed through the URL
    $brand = isset($_GET['brand']) ? trim($_GET['brand']) : '';  // Brand received via URL query parameter
    ...
    // Append location filter if set (user input, not from URL)
    if (!empty($location)) {
        $sql .= " AND location = ?";
        $params[] = $location;
        $types .= "s";
    ...
    // Append brand filter if set (from URL)
    if (!empty($brand)) {
        $sql .= " AND brand = ?";
        $params[] = $brand;
        $types .= "s";
    ...
    // Append model_name filter if set
    if (!empty($model_name)) {
        $sql .= " AND model_name = ?";
        $params[] = $model_name;
        $types .= "s";
