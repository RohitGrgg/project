...
try {
    ...
    // New CC filter added, received from URL
    $engine_displacement = isset($_GET['cc']) ? trim($_GET['cc']) : '';  // CC filter from URL

    // Other filters (user input via POST)
    $location = isset($_POST['location']) ? trim($_POST['location']) : '';  // User input for location
    $brand = isset($_POST['brand']) ? trim($_POST['brand']) : '';  // User input for brand
    $model_name = isset($_POST['model_name']) ? trim($_POST['model_name']) : '';  // User input for model name
    ...
    // Append location filter if set (user input)
    if (!empty($location)) {
        $sql .= " AND location = ?";
        $params[] = $location;
        $types .= "s";
    ...
    // Append brand filter if set (user input)
    if (!empty($brand)) {
        $sql .= " AND brand = ?";
        $params[] = $brand;
        $types .= "s";
    ...
    // Append model_name filter if set (user input)
    if (!empty($model_name)) {
        $sql .= " AND model_name = ?";
        $params[] = $model_name;
        $types .= "s";
    ...
    // Append engine displacement filter if set (from URL)
    ...
    $filters = ['location', 'brand', 'model_name', 'engine_displacement'];
    $data = [];
